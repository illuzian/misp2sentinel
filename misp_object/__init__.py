...
def determine_confidence_level(tag):
    ...
    for confidence in MISP_CONFIDENCE["matches"]:
        if confidence == confidence_tag:
            return MISP_CONFIDENCE["matches"][confidence]
    return None
...
class MISPEvent:
    def __init__(self, event, logger, misp_flatten_attributes=False):
        ...
        for label in event.get("Tag", []):
            ...
            if config.use_event_confidence and MISP_CONFIDENCE["prefix"] in label.lower():
                self.confidence = determine_confidence_level(label)
